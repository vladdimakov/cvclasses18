...
namespace cvlib
{
AdvancedMotionSegmentation::AdvancedMotionSegmentation(float refreshRate, float deviationFactor, float targetsFactor, int maxCornersNum, int minCornersNum)
    : m_refreshRate(refreshRate), m_deviationFactor(deviationFactor), m_targetsFactor(targetsFactor), m_maxCornersNum(maxCornersNum),
      m_minCornersNum(minCornersNum)
{
    m_needToInit = true;
    m_deviationImgInitValue = 256.0f / targetsFactor;
}

void AdvancedMotionSegmentation::init()
{
...
float AdvancedMotionSegmentation::getVectMedian(std::vector<float> vect)
{
...
cv::Point2f AdvancedMotionSegmentation::getPointsOffset()
{
...
cv::Point2f AdvancedMotionSegmentation::getFrameOffset()
{
    if (m_needToInit)
    {
...
void AdvancedMotionSegmentation::translateFrame(cv::Mat inputFrame, cv::Mat& outputFrame, cv::Point2f offset)
{
...
void AdvancedMotionSegmentation::translateAverageBackAndDeviationImg(cv::Point2f frameOffset)
{
...
int AdvancedMotionSegmentation::getBackgroundFactor()
{
    int histSize = 256;
    float range[] = {0, 256};
...
void AdvancedMotionSegmentation::setNeedToInit(bool needToInit)
{
    m_needToInit = needToInit;
}

void AdvancedMotionSegmentation::process(const cv::Mat& frame)
{
    if (frame.empty())
        return;
    ...
    m_binaryFrame = cv::Mat(m_currFrame8u.size(), CV_8U, cv::Scalar(255));
    m_binaryFrame.setTo(cv::Scalar(0), m_frameStaticPartMask + m_backgroundMask);
}

void AdvancedMotionSegmentation::getDeviationImage(cv::Mat& deviationImage)
{
    deviationImage = m_deviationImg * 20;
    deviationImage.convertTo(deviationImage, CV_8U);
}

void AdvancedMotionSegmentation::getBackgroundImage(cv::Mat& backgroundImage)
{
    m_averageBackImg.convertTo(backgroundImage, CV_8U);
}

void AdvancedMotionSegmentation::getBinaryImage(cv::Mat& binaryImage)
{
    m_binaryFrame.assignTo(binaryImage);
}
} // namespace cvlib
